...
export function lintDir(dir: string, expectedName?: string): LintIssue[] {
  ...
  if (!NAME_RE.test(name)) {
    issues.push({
      level: "error",
      rule: "naming",
      message: `目录名 "${name}" 不符合命名规范（小写字母/数字 + 连字符）`,
    });
  }
  ...
  if (!info.name) {
    ...
    issues.push({
      level: "warning",
      rule: "frontmatter",
      message: `frontmatter name "${info.name}" 与目录名 "${name}" 不一致`,
    });
  }
  if (!info.description) {
    ...
    if (info.description.length < 10) {
      issues.push({
        level: "warning",
        rule: "description",
        message: "description 过短，Agent 难以判断触发时机",
      });
    }
  ...
  if (lineCount > MAX_SKILL_MD_LINES) {
    issues.push({
      level: "warning",
      rule: "body",
      message: `SKILL.md 超过 ${MAX_SKILL_MD_LINES} 行，细节建议移到 references/ 按需加载`,
    });
  }
  return issues;
}
